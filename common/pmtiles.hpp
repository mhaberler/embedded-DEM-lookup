...
const uint8_t PMERR_BAD_MAGIC = -1;
const uint8_t PMERR_BAD_VERSION = -2;
const uint8_t PMERR_VARINT_TOO_LONG = -3;
const uint8_t PMERR_END_OF_BUFFER = -4;
const uint8_t PMERR_TILE_ZOOM_OVERFLOW = -5;
...
inline zxy tileid_to_zxy(uint64_t tileid, uint8_t &pmerr = pmerrno) {
    uint64_t acc = 0;
...
inline std::pair<uint64_t, uint32_t> get_tile(const std::function<std::string(const std::string &, uint8_t)> decompress, const char *pmtiles_map, uint8_t z, uint32_t x, uint32_t y, uint8_t &pmerr = pmerrno) {
